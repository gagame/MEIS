...
#define   STATION_SSID     "970"
#define   STATION_PASSWORD "0970979085"
...
void setup() {
  ...
  server.on("/memory.js", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/memory.js", "text/javascript");
  });
  server.on("/memory", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/memory.html", "text/html");
    if (request->hasArg("BROADCAST")){
      String msg = request->arg("BROADCAST");
      mesh.sendBroadcast(msg);
    }
...
void loop() {
  ...
  if(myIP != getlocalIP()){
    ...
    mesh.sendBroadcast("My IP is " + myIP.toString());
  }
}
